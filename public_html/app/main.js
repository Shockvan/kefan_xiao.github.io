...
app.controller('DashController', function($scope, sources, contributors) {
    ...
    function calculateHTTPMetrics(type, requests) {
        ...
        var requestWindow = requests.slice(-10).filter(function (req) {
        ...
        if (requestWindow.length > 0) {
            ...
            // add new var
            var mylatencyTimes = requestWindow.map(function (req){ return Math.abs(req.tresponse.getTime() - req.trequest.getTime()) ;});
            latency[type] = {
                ...
                everyspot: latencyTimes[latencyTimes.length-1]
            };
            ...
            download[type] = {
                ...
                everyspot: downloadTimes[downloadTimes.length - 1]
            };

            var durationTimes = requestWindow.map(function (req){ return req._mediaduration;});
            var throughputTrace = requestWindow.map(function (req){ return req.tarce;});
            ...
            ratio[type] = {
                ...
                everyspot: durationTimes[durationTimes.length-1]
            };
    ...
    function updateMetrics(type) {
        ...
        if (metrics && dashMetrics && $scope.streamInfo) {
            ...
            if (httpMetrics) {
                ...
                $scope[type + "Latency"] += "|"+httpMetrics.latency[type].everyspot.toFixed(3); //httpMetrics.latency[type].low.toFixed(2) + " | " + httpMetrics.latency[type].average.toFixed(2) + " | " + httpMetrics.latency[type].high.toFixed(2)+"|"+httpMetrics.latency[type].everyspot.toFixed(3);
                $scope[type + "Ratio"] += "|" + (httpMetrics.ratio[type].everyspot*bitrate/httpMetrics.download[type].everyspot).toFixed(3);
                ...
                //$scope[myVideoLatency] += "|"+httpMetrics.latency[type].everyspot.toFixed(3);
            }
